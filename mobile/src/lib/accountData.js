...
export function buildStatsFromHistory(historyRows) {
  ...
  return { completedHands, wins, losses, winRatePct, handBreakdown };
}
...
export async function ensureProfileForUser(user) {
  ...
  if (existingProfile) return { profile: existingProfile, isNew: false };

  const payload = { user_id: user.id, display_name: getDefaultDisplayName(user) || null };
...
export async function fetchAppConfig() {
  const client = requireSupabase();
  const { data, error } = await client
    .from("app_config").select("leaderboard_min_hands").eq("id", APP_CONFIG_ROW_ID).single();
  if (error) throw error;
  if (typeof data?.leaderboard_min_hands !== "number") throw new Error("Leaderboard config is missing.");
  return { leaderboardMinHands: data.leaderboard_min_hands };
}
...
export async function fetchHistory(userId) {
  ...
  const { data, error } = await client
    .from("game_sessions")
    .select("id, completed_at, outcome, player_hand_name, player_hand_category, dealer_hand_name, dealer_hand_category, dealer_won_tie, turns_played")
...
export async function saveCompletedGameRecord({ userId, summary }) {
  ...
  const { data: existing, error: existingError } = await client
    .from("game_sessions").select("id, status").eq("user_id", userId)
    .contains("metadata", { local_game_id: summary.localGameId }).limit(1);
  if (existingError) throw existingError;
  ...
  const completedFields = {
    ...
    metadata: { local_game_id: summary.localGameId, source: "client_recorded_v1" },
  };

  if (existing?.length) {
    if (existing[0].status === "completed") return existing[0];
    const { data, error } = await client
      .from("game_sessions").update(completedFields).eq("id", existing[0].id).select("id").single();
    if (error) throw error;
    return data;
  }

  const { data: sessionRow, error: sessionError } = await client
    .from("game_sessions").insert({ user_id: userId, ...completedFields }).select("id").single();
